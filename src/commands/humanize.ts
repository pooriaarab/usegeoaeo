...
export async function expandTextGlob(pattern: string, directory = process.cwd()): Promise<string[]> {
  const files = await filesIn(directory);
  const relativeFiles = files.map(file => ({ file, relative: path.relative(directory, file).replaceAll('\\', '/') }));
  const matcher = globRegExp(pattern.replaceAll('\\', '/'));
  return relativeFiles.filter(item => matcher.test(item.relative) || (!pattern.includes('/') && matcher.test(path.basename(item.relative)))).map(item => item.file);
}

export async function humanizeGlob(pattern: string, options: { directory?: string; write?: boolean } = {}): Promise<Map<string, HumanizeResult>> {
