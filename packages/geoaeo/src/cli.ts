...
program
  .name(PKG_NAME)
  .version(VERSION)
  .description("GEO and AEO site artifacts for answer engines and agents.");
...
program
  ...
  .action(async (directory = ".", options: { force?: boolean }) => {
    ...
    process.stdout.write(
      `${result.mode} project: created ${result.created.length}, skipped ${result.skipped.length}\n`,
    );
  });
  ...
  .action(async (pattern: string, options: { check?: boolean; write?: boolean }) => {
    ...
    for (const [file, result] of results) {
      ...
      if (result.findings.length)
        process.stdout.write(`${file}: ${result.findings.length} finding(s)\n`);
    }
