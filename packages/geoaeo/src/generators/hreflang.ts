...
export function generateHreflang(config: SiteConfig, locales: string[] = ['en']): string {
  ...
  const codes = [...new Set((locales.length ? locales : ['en']).filter(locale => locale && locale !== 'x-default'))];
  const tags = codes.map(
    locale => `<link rel="alternate" hreflang="${escapeAttr(locale)}" href="${escapeAttr(href)}" />`
  );
