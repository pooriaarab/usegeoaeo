...
export function howToJsonLd(config: SiteConfig, howTo: HowToInput = {}): Record<string, unknown> {
  return {
    "@context": "https://schema.org",
    "@type": "HowTo",
...
function createBreadcrumbItems(config: SiteConfig): Array<{ name: string; url: string }> {
  ...
  return [
    ...base,
    { name: config.tools[0].name, url: absoluteUrl(config.siteUrl, config.tools[0].url) },
  ];
}
