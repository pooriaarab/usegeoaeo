...
const directorySchema = z
  ...
  .optional()
  .describe(
    "Absolute path to the site root. Defaults to the MCP server's working directory, which is usually not the project you mean — pass it explicitly.",
  );
const targetSchema = z.string().describe("A local directory or an absolute http(s) URL.");
const globSchema = z
  .string()
  .describe(
    "Glob pattern matching the prose files to scan. Reads the matches; changes nothing unless write is true.",
  );
const writeSchema = z
  .boolean()
  .optional()
  .describe(
    "Default false. true rewrites matching files in place. Set true only after the user asked to change files.",
  );
const artifactSchema = genArtifactSchema.describe(
  "Which GEO or AEO artifact to generate. It is returned as text; no file is written.",
);
const artifactTypeSchema = genTypeSchema
  .optional()
  .describe("Applies only when artifact is jsonld.");
...
async function humanizeHandler({
  glob,
  write,
  directory,
}: {
  ...
  const summary = [...results.entries()].map(([file, result]) => ({
    file,
  ...
  if (error.code === "CONFIG_MISSING") {
    return {
      ...
      content: [
        {
          type: "text",
          text: `No ${CONFIG_FILENAME} in ${root}. Run geoaeo init in that directory, replace the placeholder facts, then call gen again.`,
        },
      ],
      structuredContent: {
        status: "error",
        error: { code: "CONFIG_MISSING", message: `No ${CONFIG_FILENAME}` },
      },
    };
  }
  return {
    ...
    content: [
      {
        type: "text",
        text: `${error.filename} in ${root} has no default export. Export a default config or siteConfig, then call gen again.`,
      },
    ],
    structuredContent: {
      status: "error",
...
export async function genHandler({
  artifact,
  type,
  directory,
}: {
...
function registerAuditTool(registerTool: RegisterTool) {
  registerTool(
    "audit",
    {
      title: "Audit a site",
      description:
        "Audit a live URL or local site directory for GEO and AEO gaps. Reads the target and returns a report; it changes no files. Fetches over the network when the target is a URL.",
...
function registerGenTool(registerTool: RegisterTool) {
  registerTool(
    "gen",
    {
      title: "Generate an artifact",
      description:
        "Generate one GEO or AEO artifact from the local site config. Returns the artifact as text; it does not write a file.",
      inputSchema: {
...
function registerHumanizeTool(registerTool: RegisterTool) {
  registerTool(
    "humanize",
    {
      title: "Humanize prose",
      description:
        "Find AI-writing tells in prose files. Reports findings without changing files by default; with write true it rewrites matching files in place, so ask the user first.",
...
export function createMcpServer(): McpServer {
  ...
  registerAuditTool(registerTool);
  registerGenTool(registerTool);
  registerHumanizeTool(registerTool);
