...
function installFetch(bodies: Record<string, string>): void {
  globalThis.fetch = async (input: RequestInfo | URL) => {
    const artifactPath = Object.keys(bodies).find(
      (candidate) => String(input) === `${liveUrl}${candidate}`,
    );
    return artifactPath
      ? new Response(bodies[artifactPath], { status: 200 })
