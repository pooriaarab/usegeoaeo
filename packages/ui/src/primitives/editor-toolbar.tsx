 ...
 */
export function EditorToolbar({ actions, variant = "fixed", className }: EditorToolbarProps) {
  return (
    <div
      role="toolbar"
      ...
      className={cn(
        ...
        variant === "fixed" && "border-b border-border bg-background px-2 py-1",
        variant === "floating" &&
    ...
    >
      {actions.map((action, index) => {
        ...
        const showSeparator =
          index > 0 && prevGroup !== currentGroup && prevGroup !== null && currentGroup !== null;
...
function ToolbarButton({ action }: { action: ToolbarAction }) {
  ...
  return (
    ...
    >
      ...
      {action.shortcut && <span className="sr-only">({action.shortcut})</span>}
    </button>
  );
}
