...
function calcUtilization(currentSpend: number, monthlyBudget: number) {
  const utilization = monthlyBudget > 0 ? Math.round((currentSpend / monthlyBudget) * 100) : 0;
  return { utilization, progressWidth: Math.min(utilization, 100) };
}
...
export function BudgetReachedDialog({
  ...
  const { utilization, progressWidth } = calcUtilization(currentSpend, monthlyBudget);
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogFooter>
          ...
          {onIncreaseBudget && <Button onClick={onIncreaseBudget}>Increase Budget</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
