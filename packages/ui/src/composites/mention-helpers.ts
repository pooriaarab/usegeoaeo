...
function collectUrlTokens(text: string, tokens: { index: number; length: number }[]) {
  ...
  while ((m = urlRe.exec(text)) !== null) {
    ...
    if (!overlaps) tokens.push({ index: idx, length: m[0].length, kind: 'url' as const, data: { url: m[0] } } as never);
  }
}

function tokensToSegments(text: string, tokens: { index: number; length: number; kind: string; data: unknown }[]) {
  tokens.sort((a, b) => a.index - b.index);
  const segments: Segment[] = [];
  let lastIndex = 0;
  for (const token of tokens) {
    if (token.index > lastIndex) segments.push({ type: 'text', content: text.slice(lastIndex, token.index) });
...
export function filterEntities(
  ...
  const filtered = entities.filter(
    (e) => e.displayName.toLowerCase().includes(q) || e.name.toLowerCase().includes(q) || e.category.includes(q),
  );
...
export function groupEntities(filteredEntities: MentionEntity[]) {
  ...
  for (const entity of filteredEntities) {
    if (entity.category !== currentCat) {
      if (currentCat !== null && currentItems.length > 0) groups.push({ category: currentCat, items: currentItems });
